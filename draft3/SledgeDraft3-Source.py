#SOURCE CODE CREDIT GOES TO FABIAN ROSS#
#SOURCE CODE CREDIT GOES TO FABIAN ROSS#
#SOURCE CODE CREDIT GOES TO FABIAN ROSS#
#SOURCE CODE CREDIT GOES TO FABIAN ROSS#
#SOURCE CODE CREDIT GOES TO FABIAN ROSS#
...
root = Tk()
root.title("Sledge Inventory Application")
...
def Exit():
    result = tkMessageBox.askquestion('Sledge Inventory Application', 'Are you sure you want to exit?', icon="warning")
    if result == 'yes':
        root.destroy()
        exit()
...
def ShowLoginForm():
    global loginform
    loginform = Toplevel()
    loginform.title("Sledge Inventory Application/Account Login")
    width = 600
    height = 500
    screen_width = root.winfo_screenwidth()
    screen_height = root.winfo_screenheight()
...
def Home():
    global Home
    Home = Tk()
    Home.title("Sledge Inventory Application/Home")
    width = 1024
    height = 520
    ...
    Title.pack(pady=10)
    lbl_display = Label(Title, text="Sledge Inventory Application", font=('arial', 45))
    lbl_display.pack()
    menubar = Menu(Home)
    ...
    Home.config(bg="#FFD966")

def ShowAddNew():
    global addnewform
    addnewform = Toplevel()
    addnewform.title("Sledge Inventory Application/Add new")
    width = 600
    height = 500
    screen_width = Home.winfo_screenwidth()
    screen_height = Home.winfo_screenheight()
...
def Delete():
    if not tree.selection():
       print("ERROR")
        ...
        result = tkMessageBox.askquestion('Sledge Inventory Application', 'Are you sure you want to delete this record?', icon="warning")
        if result == 'yes':
            curItem = tree.focus()
...
def ShowView():
    global viewform
    viewform = Toplevel()
    viewform.title("Sledge Inventory Application/View Product")
    width = 600
    height = 400
    screen_width = Home.winfo_screenwidth()
    screen_height = Home.winfo_screenheight()
...
def Logout():
    result = tkMessageBox.askquestion('Sledge Inventory Application', 'Are you sure you want to logout?', icon="warning")
    if result == 'yes': 
        admin_id = ""
        root.deiconify()
        Home.destroy()
